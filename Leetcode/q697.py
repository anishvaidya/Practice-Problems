...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 15 17:07:57 2020
...
def findDegree(self, nums: List[int]) -> int:
        ...
        for num in nums:
            if num not in count_dict:
                ...
                count_dict[num] += 1
            if degree < count_dict[num]:
                degree = count_dict[num]
        return degree
    def findShortestSubArray(self, nums: List[int]) -> int:
        orig_degree = self.findDegree(nums)
        window_size = orig_degree
        
        while window_size < len(nums):
            i = 0
            while i < (len(nums) - window_size + 1):
                degree = self.findDegree(nums[i: window_size + i])
                # print (str(nums[i: window_size + i]) + " + " + str(degree))
                if degree == orig_degree:
                    return window_size
                i += 1
            window_size += 1
    ...
    def findShortestSubArray(self, nums: List[int]) -> int:
        ...
        for i, num in enumerate(nums):
            if num in count_dict:
                ...
                count_dict[num] = [i]
            degree = max(degree, len(count_dict[num]))
        for num in count_dict.keys():
            if len(count_dict[num]) == degree:
                min_length = min(min_length, count_dict[num][-1] - count_dict[num][0] + 1)
        return min_length
